...
export function FreeRankingPage({ rankListEntity, initialItems, onBack }: FreeRankingPageProps) {
  ...
  const handleDropOnItem = (targetItem: ItemEntity) => {
    ...
    if (itemAbove && itemBelow) {
      ...
      const scoreAbove = itemScores.get(itemAbove.id) || 0
      const scoreBelow = itemScores.get(itemBelow.id) || 0
      ...
      // At the bottom - midpoint between item above and 0
      const scoreAbove = itemScores.get(itemAbove.id) || 0
      scoreToAssign = Math.round(((scoreAbove + 0) / 2) * 100) / 100
    } else {
